<template>
	<CustomPage>
		<view class="page_container share-withdraw">
			<view class="page_content">
				<view class="head_content">
					<CustomNavbar :title="navbarTitle" color="#fff"></CustomNavbar>
				</view>
					...
					<view class="top_card">
						<view class="line1">
							...
							<view class="right" @click="jumpView('/pages/user/share/payee')">
								<image class="image" src="/static/icons/setting.png" mode="widthFix"></image>
								<text>收款号设置</text>
							</view>
						</view>
						<view class="line2">
						...
						<view class="line3">
							...
							<view class="right" @click="typePicker = true">{{ typeText }} > </view>
						</view>
						<view class="line4">
							<u-button text="确认提现" :loading="buttonLoading" :customStyle="{ ...themeButton.general[theme], ...buttonStyle }" @click="withdrawHandle" />
						</view>
						<u-picker :show="typePicker" :columns="types" keyName="label" :closeOnClickOverlay="true" :confirmColor="themeStyle.picker[theme]" @close="typePicker = false" @cancel="typePicker = false" @confirm="selectedType"></u-picker>
					</view>
								...
								<view class="scroll_content">
									<view class="list_box" v-if="recordData.length">
										<view class="item" v-for="(item, index) in recordData" :key="index">
											...
											<view class="right" :class="[statusClass(item.status)]">{{ item.status_text }}</view>
										</view>
									</view>
...
<script>
	...
	export default {
		...
		computed: {
			...mapGetters("app", ["theme", "themeButton", "themeStyle"])
		},
		onLoad() {
			this.withdrawRule()
			this.recordList()
