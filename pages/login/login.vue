<template>
	<view class="page_content">
		<view class="head_content">
			<u-navbar @leftClick="navBack" :leftIconSize="16" leftIconColor="#000" :title="navTitle" :titleStyle="navbarTitleStyle" :bgColor="navbarBg" :placeholder="true" />
		</view>
	...
	export default {
		...
		methods: {
			...
			agreementClick(id, title) {
				...
				uni.navigateTo({
					url: `/pages/user/richtext?d=${encodeURIComponent(JSON.stringify(obj))}`
				})
			},
			// 登录提交
			loginSubmit() {
			...
			async wxoaAuth() {
				...
				if(prevPage && prevPage.__page__){
					...
					const redirect_uri = encodeURIComponent(`${this.$BASE_URL}/addons/chatgpt/user/wxOfficialAccountOauth/sign/${this.$SIGN}?payload=${payload}`);
