<template>
	<CustomPage>
		<view class="page_container page_bg helper helper-write">
			<view class="page_content">
				...
				<view class="main_content">
					<scroll-view class="scroll_view" :scroll-y="true" :scroll-top="scrollTop">
						...
						<view class="reset_box" v-if="options.isChat && relink" @click="initSocketServe(2)">已断开，点击我重新连接…</view>
					</scroll-view>
				</view>
				<view class="footer_content" v-if="options.isChat" :style="{ marginBottom: marginBottom * 2 + 'rpx' }">
	...
	export default {
		...
		data() {
			return {
				...
				// domain: this.$store.state.app.config.ws_url || '', // 连接地址
		...
		watch: {
			isLogin(newValue, oldValue) {
				...
				this.options.t && this.initSocketServe()
			},
		...
		methods: {
			...
			async send() {
				...
				const params = {}
				if(Object.keys(this.options.params)[0]) {
					params[Object.keys(this.options.params)[0]] = message
				}
				const timer = setTimeout(() => {
					this.$nextTick(() => {
						this.scrollTop += 1
